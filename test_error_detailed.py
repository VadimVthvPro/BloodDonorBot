...
def test_error_detailed():
    """Детально тестирует обработку ошибок"""
    ...
    # Тестируем обработку ошибок при работе с базой данных
    print("\nТестируем обработку ошибок при работе с БД:")
    ...
    try:
        ...
        try:
            ...
            print("  Выполнение неверного SQL -> Успешно")
        except Exception as e:
            print(f"  Выполнение неверного SQL -> Ошибка: {e}")
        
        # Попытка выполнить неверный запрос
        try:
            cursor.execute("SELECT * FROM users WHERE invalid_column = %s", ("test",))
            print("  Выполнение неверного запроса -> Успешно")
        except Exception as e:
            print(f"  Выполнение неверного запроса -> Ошибка: {e}")
